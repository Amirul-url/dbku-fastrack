...
function FirstReminderGeneratedSuccessModal({ reference, months = 3, t, onClose }) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="first-reminder-generated-success-title"
    >
      <div className="w-full max-w-[830px] rounded-lg border-2 border-slate-900 bg-white px-6 py-8 text-center shadow-xl sm:px-10">
        <img
          src="/green_tick.png"
        ...
        />
        <h2
          id="first-reminder-generated-success-title"
          className="mt-5 text-4xl font-extrabold uppercase tracking-normal text-black"
        >
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .amount-columns { display: grid; grid-template-columns: 1fr 11mm; width: 100%; align-items: center; }
    .amount-columns span { text-align: center; }
...
function isStaleRenewalReceiptNumber(app = null, receiptNo = "") {
  ...
  const receiptSequence = getReceiptSequenceValue(receiptNo);
  return (
    receiptSequence > 0 &&
    originalReceiptNumbers.some(
      (originalNo) => getReceiptSequenceValue(originalNo) === receiptSequence
    )
  );
}

function getRenewalOfficialReceiptNumber(app = null, applications = [], preferredReceiptNo = "") {
  const payment = getLicenseRenewalPayment(app);
  const candidates = [
    preferredReceiptNo,
    payment.official_receipt_draft?.receipt_no,
...
const ApprovalSupportSignatureBox = forwardRef(function ApprovalSupportSignatureBox(
  ...
  const ignorePersistedSignatureSourceRef = useRef(false);
  ...
  const [ignorePersistedSignatureSource, setIgnorePersistedSignatureSourceState] =
    useState(false);
  ...
  useEffect(() => {
    ...
    setLocalDrawSource("");
    setIgnorePersistedSignatureSource(false);
    setUploadedItems(getUploadedItemsFromSignature(value));
  }, [applicationId]);
  ...
  function setIgnorePersistedSignatureSource(shouldIgnore) {
    ignorePersistedSignatureSourceRef.current = Boolean(shouldIgnore);
    setIgnorePersistedSignatureSourceState(Boolean(shouldIgnore));
  }

  function getReusablePersistedSignatureSource() {
    if (ignorePersistedSignatureSourceRef.current) return "";
    return getDecisionLogSignatureSource(value);
  }

  function getReusablePersistedSignatureMode() {
    if (ignorePersistedSignatureSourceRef.current) return "";
    return value?.mode || "";
  }
  ...
  async function finishDraw(event) {
    ...
    const existingSignatureSource = getReusablePersistedSignatureSource();
    if (
  ...
  function clearSignature() {
    ...
    setSessionBaseSignatureSource("");
    setLocalDrawSource("");
    setIgnorePersistedSignatureSource(true);
    setUploadedItems([]);
    onChange(null);
  }
  ...
  async function handleFileChange(event) {
    ...
    try {
      ...
      if (!localUploadSessionRef.current) {
        const persistedSignatureSource = getReusablePersistedSignatureSource();
        setSessionBaseSignatureSource(
          sessionBaseSignatureSourceRef.current || persistedSignatureSource,
          sessionBaseSignatureSourceRef.current
            ? sessionBaseSignatureModeRef.current
            : getReusablePersistedSignatureMode()
        );
        setLocalDrawSource("");
      }
  ...
  const signaturePreviewSource = ignorePersistedSignatureSource
    ? ""
            ...
            <Button
              ...
              onClick={() => {
                if (
                  ...
                  setSessionBaseSignatureSource(
                    getReusablePersistedSignatureSource(),
                    getReusablePersistedSignatureMode()
                  );
                }
                setMode("draw");
...
<article class="dbku-renewal-letter">
  <style>
    ...
    .dbku-renewal-letter .no-col { width: 8mm; }
    .dbku-renewal-letter .license-col { width: 34mm; }
    .dbku-renewal-letter .center { text-align: center; }
    .dbku-renewal-letter .period-col { width: 68mm; text-align: center; white-space: nowrap; }
    .dbku-renewal-letter .amount-col { width: 36mm; text-align: right; }
  ...
  <table>
    <thead>
      <tr>
        ...
        <th class="license-col" data-renewal-editable="true">JENIS LESEN</th>
    ...
    <tbody>
      <tr>
        ...
        <td class="license-col" data-renewal-editable="true">${escapeHtml(context.licenseType)}</td>
      ...
      },
      {
        ...
        buildPayload: (app, data) => {
          ...
          const savedRenewalReceiptNo =
            ...
            payment.official_receipt_no;
          const officialReceiptNo = getRenewalOfficialReceiptNumber(
            app,
            data?.applications,
          ...
          const canUseSavedRenewalReceiptHtml =
...
function PaymentDetails({
  ...
  const renewalAdvertisementLicenseDocument =
    ...
    renewalPayment.manual_advertisement_license_draft ||
    null;
  const canDownloadGeneratedOfficialReceipt = showRenewalCompletionDocuments
    ? Boolean(renewalOfficialReceiptDocument?.document_html)
    : Boolean(manualReceipt.document_html);
  const canDownloadGeneratedAdvertisementLicense = showRenewalCompletionDocuments
    ? Boolean(renewalAdvertisementLicenseDocument?.document_html)
    : Boolean(manualLicense.document_html);
  ...
  const renewalAdvertisementLicenseDocumentRow = {
    ...
    onDownload: () =>
      printHtmlDocument(
        renewalAdvertisementLicenseDocument?.document_html ||
          getRenewalGeneratedAdvertisementLicenseDocumentHtml(app, t, applications),
  ...
  const verificationDocuments = [
    {
      ...
      canDownload: canDownloadGeneratedOfficialReceipt,
    ...
    },
    {
      ...
      canDownload: canDownloadGeneratedAdvertisementLicense,
      onDownload: showRenewalCompletionDocuments
...
function GeneratedDocumentReviewModal({ document, t, saving, onClose, onSave }) {
  ...
  const defaultDocumentZoom = getGeneratedDocumentReviewZoom(document?.scale);
  const [documentZoom, setDocumentZoom] = useState(defaultDocumentZoom);
  ...
  useEffect(() => {
    setIframeHeight(720);
    setSignatureTool("select");
    setDocumentZoom(defaultDocumentZoom);
  }, [document?.html, defaultDocumentZoom]);

  const resizeIframe = useCallback(() => {
    const frameDocument = iframeRef.current?.contentDocument;
    if (!frameDocument) return;

    const nextHeight = getGeneratedDocumentRenderedHeight(frameDocument);
    setIframeHeight(Math.ceil(nextHeight + 2));
  }, []);
  ...
  const resizeSignatureCanvas = useCallback(() => {
    ...
    const width = iframe.clientWidth || iframe.offsetWidth || 1;
    const height = Math.max(iframe.clientHeight || iframe.offsetHeight || iframeHeight, 1);
    canvas.width = Math.round(width * ratio);
    canvas.height = Math.round(height * ratio);
    canvas.style.width = `${width}px`;
    canvas.style.height = `${height}px`;
  ...
  useEffect(() => {
    if (signatureTool !== "draw") return;
    resizeSignatureCanvas();
  }, [resizeSignatureCanvas, signatureTool, iframeHeight]);

  useEffect(() => {
    window.setTimeout(resizeIframe, 0);
    window.setTimeout(resizeSignatureCanvas, 50);
  }, [documentZoom, previewWidth, resizeIframe, resizeSignatureCanvas]);
  ...
  }, [resizeSignatureCanvas, signatureTool]);
  ...
  function handleIframeLoad() {
    ...
    window.setTimeout(resizeSignatureCanvas, 350);
  }
  ...
  function changeDocumentZoom(step) {
    setDocumentZoom((currentZoom) =>
      Math.min(1.35, Math.max(0.55, Number((currentZoom + step).toFixed(2))))
    );
    window.setTimeout(resizeIframe, 0);
    window.setTimeout(resizeSignatureCanvas, 50);
  }
  ...
  function moveSignatureDraw(event) {
    if (!signatureDrawingRef.current || signatureTool !== "draw") return;
    event.preventDefault();
  ...
  function finishSignatureDraw() {
    ...
    const dataUrl = getCroppedSignatureCanvasDataUrl(signatureCanvasRef.current);
    if (!dataUrl) return;

    insertSignatureIntoGeneratedDocument(iframeRef.current?.contentDocument, document?.kind, dataUrl);
  ...
  async function handleSignatureUpload(event) {
    ...
    if (!String(file.type || "").startsWith("image/")) {
      window.alert(t("workspace.signature.imageOnly", "Please upload an image file for the signature."));
      return;
    }
    ...
    try {
      ...
      window.alert(t("workspace.signature.uploadFailed", "Could not read the signature file."));
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 px-4 py-6">
      <div className="max-h-[92vh] w-full max-w-[min(96vw,72rem)] overflow-hidden rounded-md bg-white shadow-xl">
        ...
        <div
          className="max-h-[calc(92vh-64px)] overflow-x-auto overflow-y-scroll bg-slate-100 px-4 py-5"
          style={{ scrollbarGutter: "stable" }}
        >
          <div
            className={canUseSignatureTools ? "grid gap-3 md:grid-cols-[48px_minmax(0,1fr)]" : "grid justify-center"}
            style={{
              minWidth: canUseSignatureTools
                ? `${previewWidth + 60}px`
                : `max(100%, ${previewWidth}px)`,
            }}
          >
            {canUseSignatureTools && (
              <div className="sticky top-0 z-20 flex h-max flex-row gap-2 rounded-md border border-slate-200 bg-white p-2 shadow-sm md:flex-col">
                <button
                  type="button"
                  onClick={() => setSignatureTool("select")}
                  className={`inline-flex h-9 w-9 items-center justify-center rounded-md border text-slate-700 transition ${
                    signatureTool === "select"
                      ? "border-emerald-600 bg-emerald-50 text-emerald-700"
                      : "border-slate-200 bg-white hover:bg-slate-50"
                  }`}
                  title={t("workspace.signature.selectTool", "Select")}
                  aria-label={t("workspace.signature.selectTool", "Select")}
                >
                  <Icon name="near_me" className="text-[18px]" />
                </button>
                <button
                  type="button"
                  onClick={() => setSignatureTool("draw")}
                  className={`inline-flex h-9 w-9 items-center justify-center rounded-md border text-slate-700 transition ${
                    signatureTool === "draw"
                      ? "border-emerald-600 bg-emerald-50 text-emerald-700"
                      : "border-slate-200 bg-white hover:bg-slate-50"
                  }`}
                  title={t("workspace.signature.draw", "Draw Signature")}
                  aria-label={t("workspace.signature.draw", "Draw Signature")}
                >
                ...
                <button
                  ...
                  title={t("workspace.signature.upload", "Upload File")}
                  aria-label={t("workspace.signature.upload", "Upload File")}
                >
                  <Icon name="upload_file" className="text-[18px]" />
                </button>
                <button
                  type="button"
                  onClick={() => removeSignatureFromGeneratedDocument(iframeRef.current?.contentDocument, document?.kind)}
                  className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-700 transition hover:bg-slate-50"
                  title={t("workspace.signature.deleteImage", "Delete image")}
                  aria-label={t("workspace.signature.deleteImage", "Delete image")}
                >
            ...
            <div className="min-w-0 pb-3">
              <div className="relative mx-auto w-fit">
                <iframe
                  ...
                  style={{ height: `${iframeHeight}px`, width: `${previewWidth}px` }}
                  className="block max-w-none border-0 bg-transparent"
                />
                {signatureTool === "draw" && (
                  <canvas
                    ref={signatureCanvasRef}
                    className="absolute left-0 top-0 z-10 cursor-crosshair touch-none"
...
function getGeneratedDocumentSignatureTarget(frameDocument, kind = "") {
  if (!frameDocument) return null;

  if (kind === "advertisement_license") {
    return frameDocument.querySelector(".signature-row > div:first-child .dot-line");
  }

  return frameDocument.querySelector(".signature .line");
}

function prepareGeneratedDocumentSignatureTarget(frameDocument, kind = "") {
  const target = getGeneratedDocumentSignatureTarget(frameDocument, kind);
  if (!target) return;

  target.setAttribute("data-signature-drop-target", "true");
  target.style.position = "relative";
  target.style.overflow = "visible";
}

function insertSignatureIntoGeneratedDocument(frameDocument, kind = "", dataUrl = "") {
  const target = getGeneratedDocumentSignatureTarget(frameDocument, kind);
  if (!target || !dataUrl) return false;

  prepareGeneratedDocumentSignatureTarget(frameDocument, kind);
  target.querySelectorAll("[data-generated-signature-image='true']").forEach((node) => node.remove());

  const image = frameDocument.createElement("img");
  image.src = dataUrl;
  image.alt = "Digital signature";
  image.setAttribute("data-generated-signature-image", "true");
  image.style.position = "absolute";
  image.style.left = "50%";
  image.style.bottom = kind === "advertisement_license" ? "-1mm" : "-1.5mm";
  image.style.transform = "translateX(-50%)";
  image.style.width = kind === "advertisement_license" ? "50mm" : "43mm";
  image.style.maxHeight = kind === "advertisement_license" ? "20mm" : "18mm";
...
function removeSignatureFromGeneratedDocument(frameDocument, kind = "") {
  const target = getGeneratedDocumentSignatureTarget(frameDocument, kind);
  target?.querySelectorAll("[data-generated-signature-image='true']").forEach((node) => node.remove());
}

function getCroppedSignatureCanvasDataUrl(canvas) {
  if (!canvas) return "";
  ...
  output
    .getContext("2d")
    .drawImage(canvas, minX, minY, output.width, output.height, 0, 0, output.width, output.height);
  return output.toDataURL("image/png");
}

function getGeneratedDocumentReviewZoom(value) {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return 0.72;
  return Math.min(1.35, Math.max(0.55, numeric));
}

function getGeneratedDocumentRenderedHeight(frameDocument) {
  if (!frameDocument) return 720;

  const pages = Array.from(
    frameDocument.querySelectorAll(".receipt-page, .page, .ad-license-page")
  );
  const pageBounds = pages
    .map((page) => page.getBoundingClientRect())
    .filter((rect) => rect && rect.height > 0);

  if (pageBounds.length > 0) {
    const top = Math.min(...pageBounds.map((rect) => rect.top));
    const bottom = Math.max(...pageBounds.map((rect) => rect.bottom));
    return Math.max(1, bottom - top);
  }

  const bodyRect = frameDocument.body?.getBoundingClientRect();
  if (bodyRect?.height) return Math.max(1, bodyRect.height);
...
function BillDocumentSlot({
  ...
  const displayName =
    ...
    (canEdit
      ? t(
          "workspace.payment.billGeneratedWithLetter",
          "Please review the auto-generated bill before sending it to the applicant."
        )
      : "");
...
function getRenewalCompletionDocumentApp(app = null, applications = [], preferredReceiptNo = "") {
  const today = new Date();
  const payment = getLicenseRenewalPayment(app);
  const approvalLetter = app?.form_data?.approval_letter || {};
  const manualReceipt = approvalLetter.manual_receipt || {};
...
async function printRenewalGeneratedOfficialReceiptDocument(app, t, applications = []) {
  try {
    await printHtmlDocument(
      getRenewalGeneratedOfficialReceiptDocumentHtml(app, t, applications),
      `${getApplicationReference(app)} ${t("workspace.payment.manual.officialReceiptTitle", "Official Receipt")}`
...
async function printRenewalGeneratedAdvertisementLicenseDocument(app, t, applications = []) {
  try {
    await printHtmlDocument(
      getRenewalGeneratedAdvertisementLicenseDocumentHtml(app, t, applications),
      `${getApplicationReference(app)} ${t("workspace.license.documentTitle", "Advertisement License")}`
...
function normalizeAdvertisementLicenseDocumentSpacing(html) {
  const normalizedHtml = ensureAdvertisementLicenseComputerNoticeHtml(
    removeAdvertisementLicenseLegacySignatureHtml(String(html || ""))
      .replace(
        /\.ad-license-page\s*\{([^}]*)\}/,
        (match, body) =>
          /position\s*:/.test(body)
  ...
  if (/\.computer-notice\b/.test(normalizedHtml)) return normalizedHtml;
...
function buildAdvertisementLicenseComputerNoticeHtml() {
  return `<footer class="computer-notice">
      <p>Notis ini adalah cetakan komputer. Tiada tandatangan diperlukan.</p>
      <p>Sila abaikan surat ini sekiranya pembaharuan telah dibuat.</p>
    </footer>`;
}

function removeAdvertisementLicenseLegacySignatureHtml(html = "") {
  return String(html || "")
