...
function AdminOverviewStatusCard({ label, value, icon, tone, compact = false }) {
  ...
  const toneClassName = tones[tone] || tones.slate;

  return (
    <div
      className="relative min-h-[94px] overflow-hidden rounded-md border border-slate-200 bg-white px-4 py-3 text-left shadow-sm"
      aria-label={`${label}: ${value}`}
    >
      <span className={`absolute inset-y-0 left-0 w-1 ${toneClassName.accent}`} />
      <div className="flex items-start justify-between gap-3 pl-1">
        ...
        <span
          className={`material-symbols-outlined shrink-0 rounded-md p-2 text-[20px] ${toneClassName.icon}`}
        >
          {icon}
        </span>
      </div>
